...
export default async function Account(){
  ...
  return <><main className="section"><div className="container"><div className="sectionHead"><div><span className="muted">ACCOUNT</span><h1 className="h2">Welcome, {u.name}.</h1></div><form action="/api/auth/logout" method="post"><button className="btn secondary">Sign out</button></form></div><div className="grid accountStats"><div className="card stat"><span className="muted">Orders</span><strong>{orderCount}</strong></div><div className="card stat"><span className="muted">Saved addresses</span><strong>{addresses.length}</strong></div><div className="card stat"><span className="muted">Wallet</span><strong>{money(walletBalance,walletCurrency)}</strong></div><div className="card stat"><span className="muted">Coins</span><strong>{coinBalance.toLocaleString()}</strong></div></div><div className="card" style={{padding:20,marginBottom:20}}><div className="sectionHead small"><div><span className="muted">REWARDS</span><h3>Your wallet & coins</h3><p className="muted">Use wallet credit for Wallet checkout. Every 1 coin is worth 0.01 in store currency.</p></div><Link href="/account/wallet" className="btn secondary">View wallet</Link></div><div className="grid twoColumn"><div><span className="muted">Available wallet</span><div style={{fontSize:28,fontWeight:700,marginTop:4}}>{money(walletBalance,walletCurrency)}</div></div><div><span className="muted">Available coins</span><div style={{fontSize:28,fontWeight:700,marginTop:4}}>{coinBalance.toLocaleString()}</div></div></div></div><div className="grid twoColumn"><section><div className="sectionHead small"><h3>Recent orders</h3><span className="muted">Showing {orders.length} of {orderCount}</span></div>{!orders.length?<div className="card empty"><p className="muted">No orders yet.</p><Link href="/shop" className="btn">Start shopping</Link></div>:<div className="grid" style={{gap:10}}>{orders.map(o=><Link href={`/account/orders/${o.orderNumber}`} className="card orderCard" key={o.id}><div><strong>#{o.orderNumber}</strong><span className="muted">{o.createdAt.toLocaleDateString()}</span></div><div><span className="pill">{o.status}</span><strong>{money(o.grandTotal,o.currency)}</strong></div></Link>)}</div>}</section><AddressBook addresses={addresses}/></div></div></main><Footer/></>
}

...
export default async function Admin() {
  ...
  return <div className="adminDashboard">
    ...
    <div className="dashboardHero"><div><div className="eyebrow">STORE OVERVIEW</div><h2>Dashboard</h2><p>Monitor revenue, orders, customers and inventory from one workspace.</p></div><div className="dashboardActions"><Link className={`${ui.btn} ${ui.btnSecondary}`} href="/">View storefront</Link><Link className={ui.btn} href="/admin/products/new">Add product</Link></div></div>
    ...
    <div className="dashboardGridPro">
      <section className="dashboardPanel"><div className="dashboardPanelHead"><div><h3>Needs attention</h3><p>Operational items worth checking now.</p></div><span className={ui.pill}>Live</span></div><div className="attentionGrid"><Link href="/admin/orders?status=PENDING" className="attentionCard pending"><strong>{newOrders}</strong><span>Pending orders</span></Link><Link href="/admin/inventory" className="attentionCard low"><strong>{lowStock}</strong><span>Low-stock items</span></Link><Link href="/admin/reviews" className="attentionCard reviews"><strong>{pendingReviews}</strong><span>Reviews to approve</span></Link></div></section>
      <section className="dashboardPanel"><div className="dashboardPanelHead"><div><h3>Top products</h3><p>Units sold this month.</p></div><Link className="dashboardLink" href="/admin/reports">Analytics</Link></div>{topTyped.length ? topTyped.map(x => <div className="topProduct" key={x.productId}><div style={{minWidth:0}}><div className="topProductName">{nm.get(x.productId) || 'Unknown product'}</div><div className="mutedSmall">Best sellers by quantity</div></div><span className="topProductQty">{x._sum.quantity || 0}</span></div>) : <div className={ui.empty}>No product sales this month.</div>}</section>
    </div>
    <section className="recentPanel"><div className="dashboardPanelHead"><div><h3>Recent orders</h3><p>Latest activity across your store.</p></div><Link className="dashboardLink" href="/admin/orders">View all</Link></div><div className="recentTableWrap"><table className={`${ui.table} recentTable`}><thead><tr><th>Order</th><th>Customer</th><th>Items</th><th>Total</th><th>Status</th></tr></thead><tbody>{orders.map(o => <tr key={o.id}><td><Link className={ui.textButton} href={`/admin/orders/${o.id}`}>#{o.orderNumber}</Link></td><td>{o.user?.name || o.email}</td><td>{o.items.reduce((a, x) => a + x.quantity, 0)}</td><td>{money(o.grandTotal, o.currency)}</td><td><span className={ui.pill}>{o.status as OrderStatus}</span></td></tr>)}</tbody></table></div></section>

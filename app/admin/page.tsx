...
export default async function Admin() {
  ...
  const [products, ordersToday, customers, revenueOrders, orders, inventoryRows, pendingReviews, newOrders, top] = await Promise.all([
    ...
    db.orderItem.groupBy({
      by: ['productId'],
      where: { order: { status: { notIn: [OrderStatus.CANCELLED, OrderStatus.REFUNDED] }, createdAt: { gte: month } } },
      _sum: { quantity: true },
  ...
  return <div>
    ...
    <div className="grid dashboardGrid">
      ...
      <section className="card adminPanel"><div className="sectionHead small"><h3>Top products this month</h3><Link className="textLink" href="/admin/analytics">Analytics →</Link></div>{top.length ? top.map(x => <div className="summaryLine" key={x.productId}><span>{nm.get(x.productId) || 'Unknown product'}</span><strong>{x._sum.quantity || 0}</strong></div>) : <div className="emptyInline">No product sales this month.</div>}</section>
    </div>
    <section className="card adminPanel"><div className="sectionHead small"><h3>Recent orders</h3><Link className="textLink" href="/admin/orders">View all →</Link></div><div className="cardInnerTable"><table className="table"><thead><tr><th>Order</th><th>Customer</th><th>Items</th><th>Total</th><th>Status</th></tr></thead><tbody>{orders.map(o => <tr key={o.id}><td><Link className="textLink" href={`/admin/orders/${o.id}`}>#{o.orderNumber}</Link></td><td>{o.user?.name || o.email}</td><td>{o.items.reduce((a, x) => a + x.quantity, 0)}</td><td>{money(o.grandTotal, o.currency)}</td><td><span className="pill">{o.status as OrderStatus}</span></td></tr>)}</tbody></table></div></section>

...
export async function POST(req: Request) {
  try {
    ...
    const row = await db.abandonedCheckout.upsert({ where: { token }, update: { customerId: b.customerId ? String(b.customerId) : null, email: b.email ? String(b.email).trim() : null, cartJson, subtotal: Math.max(0, Math.trunc(Number(b.subtotal) || 0)), currency: String(b.currency || process.env.NEXT_PUBLIC_CURRENCY || 'USD'), recoveryUrl: b.recoveryUrl ? String(b.recoveryUrl) : null, status: 'OPEN', lastActivity: new Date() }, create: { token, customerId: b.customerId ? String(b.customerId) : null, email: b.email ? String(b.email).trim() : null, cartJson, subtotal: Math.max(0, Math.trunc(Number(b.subtotal) || 0)), currency: String(b.currency || process.env.NEXT_PUBLIC_CURRENCY || 'USD'), recoveryUrl: b.recoveryUrl ? String(b.recoveryUrl) : null } })

    ...
    await db.$transaction(async tx => {
      if (type === 'categories') {
        ...
        for (const r of rows) {
          ...
          await tx.productTag.deleteMany({ where: { productId: product.id } }); const tags = split(r.tags); if (tags.length) await tx.productTag.createMany({ data: tags.map(value => ({ productId: product.id, value })), skipDuplicates: true })

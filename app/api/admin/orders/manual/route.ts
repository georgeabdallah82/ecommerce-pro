...
export async function POST(req: Request) {
  try {
    ...
    for (const item of items) {
      ...
      if (quantity < 1 || quantity > 99) return json({ error: 'Quantity must be between 1 and 99' }, { status: 400 })
      const key = `${productId}:${variantId || ''}`
      const current = merged.get(key)
      const nextQty = (current?.quantity || 0) + quantity
      if (nextQty > 99) return json({ error: 'Maximum quantity per line is 99' }, { status: 400 })
      merged.set(key, { productId, variantId, quantity: nextQty })
    }
    ...
    for (const line of merged.values()) {
      ...
      normalized.push({ productId: p.id, variantId: variant?.id ?? null, name: p.name + (variant ? ` — ${variant.name}` : ''), sku: variant?.sku ?? p.sku, quantity: line.quantity, unitPrice, totalPrice: unitPrice * line.quantity })
    }
    ...
    const address = {
      ...
      line1: String(body.line1 || '').trim(), line2: String(body.line2 || '').trim(),
      city: String(body.city || '').trim(), region: String(body.region || '').trim(),
      postalCode: String(body.postalCode || '').trim(), country: String(body.country || 'Lebanon').trim(), phone: phone || '',
    }
    ...
    const order = await db.$transaction(async tx => {
      for (const line of normalized) await reserveStock(tx, byId.get(line.productId)!, line.variantId, line.quantity, orderNumber)
      return tx.order.create({
        data: {
          orderNumber, userId: existingUser?.id ?? null, email: existingUser?.email ?? email, phone,
    ...
    const message = e instanceof Error ? e.message : 'Unable to create manual order'

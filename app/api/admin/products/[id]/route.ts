    ...
    const product=await db.$transaction(async tx=>{
      ...
      if(Array.isArray(b.variants)){
        ...
        for(const v of b.variants){
          ...
          else if(qty!==null){if(inv)await tx.inventoryItem.update({where:{id:inv.id},data:{quantity:Math.max(inv.reserved,qty),lowStockThreshold:Math.max(0,Math.trunc(Number(v.lowStockThreshold)||inv.lowStockThreshold)),location:String(v.location||inv.location||'Main')}});else await tx.inventoryItem.create({data:{productId:id,variantId:variant.id,quantity:qty,lowStockThreshold:Math.max(0,Math.trunc(Number(v.lowStockThreshold)||5)),location:String(v.location||'Main')}})}
        }
      }
      return p
    })

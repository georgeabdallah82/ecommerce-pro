  ...
  try {
    ...
    const p = await db.$transaction(async tx => {
      const created = await tx.product.create({ data: {
        ...
        salesChannelsJson: b.salesChannelsJson ? String(b.salesChannelsJson) : JSON.stringify(['online_store']), productTemplate: b.productTemplate ? String(b.productTemplate) : 'product', publishedAt: b.status === 'ACTIVE' ? (b.publishedAt ? new Date(b.publishedAt) : new Date()) : null,
      ...
      for (let index = 0; index < variants.length; index += 1) {
        ...
        const variant = await tx.productVariant.create({ data: {
          ...
          optionJson: typeof v.optionJson === 'string' ? v.optionJson : JSON.stringify(v.options || {}),
        ...
        if (!sharedPool) await tx.inventoryItem.create({ data: { productId: created.id, variantId: variant.id, quantity: Math.max(0, Math.trunc(Number(v.quantity) || 0)), reserved: 0, lowStockThreshold: Math.max(0, Math.trunc(Number(v.lowStockThreshold) || 5)), location: v.location || 'Main' } })
      }
      return created
    })

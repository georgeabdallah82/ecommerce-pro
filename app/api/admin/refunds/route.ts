...
/**
 * Creates an auditable refund transaction and derives the order payment status
 * from the total refunded amount. This endpoint is intentionally provider-neutral:
 * the actual gateway call can be added later, while manual/COD refunds remain safe.
 */
export async function POST(req: Request) {
  try {
    ...
    if (!orderId || !Number.isInteger(requestedAmount) || requestedAmount <= 0) {
      return json({ error: 'A valid orderId and positive integer refund amount are required' }, { status: 400 })
    }
    ...
    const result = await db.$transaction(async tx => {
      ...
      if (requestedAmount > remaining) {
        throw new Error(`Refund cannot exceed the remaining refundable amount of ${remaining}`)
      }
      ...
      const transaction = await tx.paymentTransaction.create({
        data: {
          ...
          rawJson: JSON.stringify({ reason: body.reason || null, actorId: actor.id }),
        },
      })
          ...
          events: {
            create: {
              ...
              message: paymentStatus === 'REFUNDED'
                ? `Order fully refunded (${requestedAmount} ${order.currency}).`
                : `Order partially refunded (${requestedAmount} ${order.currency}).`,
            },
          },
        },
      })
    ...
    await audit(actor.id, 'order.refunded', 'Order', orderId, {
      amount: requestedAmount,

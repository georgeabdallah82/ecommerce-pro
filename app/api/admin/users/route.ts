...
const visibleStaffRoles: Role[] = ['SUPER_ADMIN', 'ADMIN', 'MANAGER', 'SUPPORT', 'EDITOR']
...
export async function GET() {
  try {
    ...
    const users = await db.user.findMany({
      where: { role: { in: visibleStaffRoles } },
    ...
    return json({ error: 'Unable to load staff users' }, { status: 500, headers: { 'Cache-Control': 'private, no-store' } })
  }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    // Customer accounts are managed exclusively from Customers and cannot be promoted into staff.
    if (target.role === 'CUSTOMER') return json({ error: 'Customer accounts must be managed from Customers' }, { status: 403 })
    if (b.role === 'CUSTOMER') return json({ error: 'Customer accounts must be managed from Customers' }, { status: 400 })
    ...
    if (b.phone !== undefined) data.phone = String(b.phone || '').trim().slice(0, 50) || null
    if (b.password) {
    ...
    return json({ error: 'Unable to update staff user' }, { status: 500, headers: { 'Cache-Control': 'private, no-store' } })
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const u = await db.user.create({ data: { name: String(b.name || 'Staff').trim().slice(0, 120), email, passwordHash: await hashPassword(password), role: requestedRole }, select: safeUserSelect })
    ...
    return json({ error: 'Unable to create staff user' }, { status: 500, headers: { 'Cache-Control': 'private, no-store' } })
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    return json({ error: 'Unable to remove staff user' }, { status: 500, headers: { 'Cache-Control': 'private, no-store' } })
  }
}

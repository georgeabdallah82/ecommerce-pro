...
import { getPaymentProvider, paymentReturnToken } from '@/lib/payments'
...
export async function POST(req: Request) {
  try {
    ...
    for(const raw of merged.values()){const p=byId.get(raw.productId)!;const variant=raw.variantId?p.variants.find(v=>v.id===raw.variantId):undefined;if(raw.variantId&&!variant)return json({error:`Invalid variant for ${p.name}`},{status:400});if(p.trackInventory&&!p.continueSellingWhenOutOfStock){const dedicated=raw.variantId?p.inventory.filter(x=>x.variantId===raw.variantId):[];const stockRows=dedicated.length?dedicated:p.inventory.filter(x=>!x.variantId);const available=stockRows.reduce((s,x)=>s+x.quantity-x.reserved,0);if(available<raw.quantity)return json({error:`Not enough stock for ${p.name}`},{status:409})}const unitPrice=variant?.price??p.basePrice;subtotal+=unitPrice*raw.quantity;normalized.push({productId:p.id,variantId:variant?.id??null,name:p.name+(variant?` — ${variant.name}`:''),sku:variant?.sku??p.sku,quantity:raw.quantity,unitPrice,totalPrice:unitPrice*raw.quantity})}

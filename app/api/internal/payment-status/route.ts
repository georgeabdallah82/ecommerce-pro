...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!orderId || !externalId || !Object.values(PaymentStatus).includes(status)) return Response.json({ error: 'orderId, externalId and a valid payment status are required' }, { status: 400 })
    ...
    const result = await db.$transaction(async tx => {
      ...
      if (status === PaymentStatus.PARTIALLY_REFUNDED && (order.paymentStatus !== PaymentStatus.PAID || amount >= remainingRefundable)) throw new Error('Invalid partial refund amount')
      ...
      await tx.paymentTransaction.create({ data: { orderId: order.id, provider: typeof body.provider === 'string' && body.provider.trim() ? body.provider.trim().slice(0, 80) : 'external', externalId, status: transactionStatus, amount, currency: order.currency, rawJson: JSON.stringify(body).slice(0, 20000) } })

...
export default function Checkout() {
  ...
  useEffect(()=>{
    ...
    fetch('/api/store/settings',{cache:'no-store'}).then(async response=>{
      ...
      if(!response.ok) throw new Error(data.error||'Unable to load checkout settings')
      if(active) setSettings(data.settings||defaultSettings)
    }).catch(e=>{if(active){setSettings(defaultSettings);setSettingsError(e instanceof Error?e.message:'Unable to load checkout settings')}})
    return ()=>{active=false}
  },[])
  ...
  useEffect(()=>{
    if(enabledMethods.length && !enabledMethods.some(([value])=>value===paymentMethod)) setPaymentMethod(enabledMethods[0][0])
  },[enabledMethods.length,paymentMethod])
  ...
  return <main className="section"><div className="container split">
    <form className="card checkoutForm" onSubmit={submit}>
      ...
      {settings?.checkout.guestCheckout===false && <div className="alert danger">Guest checkout is disabled. <Link className="textLink" href="/account/login">Sign in</Link> to continue.</div>}
      ...
      <button className="btn" type="submit" disabled={loading || !items.length || !settings || enabledMethods.length===0 || settings?.checkout.guestCheckout===false} aria-busy={loading}>{loading ? 'Placing order…' : 'Place order'}</button>

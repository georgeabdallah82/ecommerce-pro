...
import './storefront-home-ux-v8.css'
import './storefront-mobile-drawer-ux-v8.css'
import './storefront-production-ux-v9.css'
import './storefront-mobile-ux-v10.css'
import './storefront-mobile-production-v11.css'
import './storefront-production-ux-v12.css'
import './storefront-production-ux-v13.css'
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const vars = {
    ...
    '--store-primary': theme.colors.primary, '--store-button-text': theme.colors.buttonText, '--store-border': theme.colors.border, '--store-max': `${theme.layout.maxWidth}px`,
    ...
    '--store-font-heading': readableFontStack, '--store-font-body': readableFontStack, '--store-animation-duration': `${theme.animations?.duration || 420}ms`,
  ...
  return <html lang="en"><head><script dangerouslySetInnerHTML={{__html:adminThemeScript}}/><link rel="stylesheet" href="/theme-fallback.css?v=13"/>{theme.faviconUrl && <link rel="icon" href={theme.faviconUrl}/>} {theme.customCss && <style dangerouslySetInnerHTML={{ __html: theme.customCss }}/>}</head><body style={vars} className={theme.animations?.enabled ? 'animations-enabled' : ''}><CartProvider><LiveVisitorTracker/><StoreNavRuntime theme={theme} navigation={navigation}/><StoreNavScroll/>{children}</CartProvider></body></html>
}

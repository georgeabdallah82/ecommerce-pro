...
export default async function ProductPage({
  ...
  const templates =
    Array.isArray(theme.editorTemplates?.Product) &&
    theme.editorTemplates.Product.length
      ? theme.editorTemplates.Product
      : [
          {
            id: 'announcement',
            type: 'announcement',
  ...
  const footerEnabled = templates.some(
    (section: any) =>
      section.type === 'footer' &&
  ...
  const sharedAvailable = sharedRows.reduce(
    (sum, inventory) => sum + inventory.quantity - inventory.reserved,
    0,
  )

  const variantAvailability = product.variants.map((variant) => {
    const dedicated = variant.inventory
    const available = dedicated.length
      ? dedicated.reduce(
          (sum, inventory) => sum + inventory.quantity - inventory.reserved,
          0,
        )
  ...
  const productAvailable = product.variants.length
    ? Math.max(
        0,
        ...variantAvailability.map((variant) => variant.available),
      )

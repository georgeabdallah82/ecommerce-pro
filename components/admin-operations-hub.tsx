...
function Card({ icon: Icon, title, count, tone = 'neutral', href }: { icon: any; title: string; count: number | string; tone?: string; href?: string }) {
  const body = <><div className="opsCardIcon"><Icon size={17}/></div><div><div className="muted opsLabel">{title}</div><strong className="opsCount">{count}</strong></div></>
  return href ? <Link href={href} className={`opsCard ${tone}`}>{body}</Link> : <div className={`opsCard ${tone}`}>{body}</div>
}
...
export default function AdminOperationsHub() {
  ...
  return <div className="operationsPage">
    ...
    {loading ? <div className="card opsLoading">Loading operations…</div> : <>
      ...
      {tab === 'orders' && <div className="opsGrid wide"><div className="card opsPanel"><div className="inline" style={{justifyContent:'space-between'}}><h3>Draft orders</h3><Link className="textLink" href="/admin/draft-orders">Open drafts</Link></div><p className="muted">Orders created in the admin before becoming completed orders.</p><div className="opsList">{data.drafts.slice(0, 12).map((x:any)=><Link className="opsRow" href={`/admin/draft-orders/${x.id}`} key={x.id}><strong>{x.orderNumber || x.id.slice(0,8)}</strong><span>{x.status}</span><span>{x.currency} {((x.grandTotal || 0)/100).toFixed(2)}</span></Link>)}{!data.drafts.length && <div className="empty">No draft orders found.</div>}</div></div><div className="card opsPanel"><div className="inline" style={{justifyContent:'space-between'}}><h3>Returns</h3><Link className="textLink" href="/admin/returns">Open returns</Link></div><p className="muted">Return requests initiated against shipped or delivered orders.</p><div className="opsList">{data.returns.slice(0, 12).map((x:any)=><div className="opsRow" key={x.id}><strong>{x.order?.orderNumber || x.orderId.slice(0,8)}</strong><span>{x.status}</span><span>{x.order?.currency || ''} {((x.refundAmount || 0)/100).toFixed(2)}</span></div>)}{!data.returns.length && <div className="empty">No return requests found.</div>}</div></div><div className="card opsPanel"><h3>Abandoned checkouts</h3><p className="muted">Recovery opportunities from incomplete carts.</p><div className="opsList">{data.abandoned.slice(0, 12).map((x:any)=><div className="opsRow" key={x.id}><strong>{x.email || 'Guest checkout'}</strong><span>{x.status}</span><span>{x.currency} {((x.subtotal || 0)/100).toFixed(2)}</span></div>)}{!data.abandoned.length && <div className="empty">No abandoned checkouts found.</div>}</div></div></div>}

      {tab === 'customers' && <div className="opsGrid wide"><div className="card opsPanel"><div className="inline" style={{justifyContent:'space-between'}}><h3>Gift cards</h3><Link className="textLink" href="/admin/gift-cards">Open gift cards</Link></div><p className="muted">Balance and status overview.</p><div className="opsList">{data.giftCards.slice(0, 12).map((x:any)=><div className="opsRow" key={x.id}><strong>•••• {x.last4 || ''}</strong><span>{x.status}</span><span>{x.currency} {((x.balance || 0)/100).toFixed(2)}</span></div>)}{!data.giftCards.length && <div className="empty">No gift cards found.</div>}</div></div><div className="card opsPanel"><h3>Customer operations</h3><p className="muted">Tags, segments and store credit are available through the customer area.</p><div className="opsActions"><a className="btn" href="/admin/customers">Customers</a><a className="btn secondary" href="/admin/customer-segments">Segments</a><a className="btn secondary" href="/admin/gift-cards">Gift cards</a></div></div></div>}

...
import { Check, ChevronDown, Copy, ExternalLink, FileText, MoreHorizontal, PackageCheck, Plus, RefreshCw, Search, Truck, X } from 'lucide-react'
...
type Modal =
  | { type: 'cancel'; order: any }
  | { type: 'refund'; order: any }
  | { type: 'tracking'; order: any }
  | null
...
function tabCount(rows: any[], id: string) {
  if (id === 'all') return rows.length
  if (id === 'unfulfilled') return rows.filter(o => !['SHIPPED','DELIVERED','CANCELLED','REFUNDED'].includes(o.status)).length
  if (id === 'unpaid') return rows.filter(o => ['UNPAID','PENDING'].includes(o.paymentStatus)).length
  if (id === 'fulfilled') return rows.filter(o => ['SHIPPED','DELIVERED'].includes(o.status)).length
  if (id === 'cancelled') return rows.filter(o => ['CANCELLED','REFUNDED'].includes(o.status)).length
  return 0
}
...
export function OrdersAdminPro({ initial }: { initial: any[] }) {
  ...
  const shown = useMemo(() => {
    ...
    return rows.filter(o => {
      const matchesQuery = !needle || `${o.orderNumber} ${o.email} ${o.phone || ''} ${o.user?.name || ''}`.toLowerCase().includes(needle)
      const matchesFilter = !filter || o.status === filter
      const matchesTab = tab === 'all'
        || (tab === 'unfulfilled' && !['SHIPPED','DELIVERED','CANCELLED','REFUNDED'].includes(o.status))
        || (tab === 'unpaid' && ['UNPAID','PENDING'].includes(o.paymentStatus))
        || (tab === 'fulfilled' && ['SHIPPED','DELIVERED'].includes(o.status))
  ...
  async function doRefund() {
    ...
    try {
      ...
      await refresh(); setModal(null); setRefundAmount(''); setNotice('Refund issued successfully.')
  ...
  async function bulkStatus(status: string) {
    ...
    try {
      ...
      await refresh(); setNotice(`${selected.length} order${selected.length === 1 ? '' : 's'} updated.`)
  ...
  return <div className="ordersProPage catalogPage">
    ...
    <div className="card orderViews">
      <button className={tab === 'all' ? 'active' : ''} onClick={() => setTab('all')}>All <span>{tabCount(rows, 'all')}</span></button>
      <button className={tab === 'unfulfilled' ? 'active' : ''} onClick={() => setTab('unfulfilled')}>Unfulfilled <span>{tabCount(rows, 'unfulfilled')}</span></button>
      <button className={tab === 'unpaid' ? 'active' : ''} onClick={() => setTab('unpaid')}>Unpaid <span>{tabCount(rows, 'unpaid')}</span></button>
      <button className={tab === 'fulfilled' ? 'active' : ''} onClick={() => setTab('fulfilled')}>Fulfilled <span>{tabCount(rows, 'fulfilled')}</span></button>
      <button className={tab === 'cancelled' ? 'active' : ''} onClick={() => setTab('cancelled')}>Cancelled <span>{tabCount(rows, 'cancelled')}</span></button>
    </div>
      ...
      <div className="tableWrap"><table className="table productTable ordersTable"><thead><tr><th><input aria-label="Select all" type="checkbox" checked={allShownSelected} onChange={() => setSelected(allShownSelected ? selected.filter(id => !shown.some(o => o.id === id)) : [...new Set([...selected, ...shown.map(o => o.id)])]}/></th><th>Order</th><th>Customer</th><th>Items</th><th>Total</th><th>Payment</th><th>Fulfillment</th><th></th></tr></thead>
        <tbody>{shown.map(o => {
          ...
          return <tr key={o.id} className={isSelected ? 'selectedRow' : ''}>
            <td><input aria-label={`Select order ${o.orderNumber}`} type="checkbox" checked={isSelected} onChange={() => toggleSelection(o.id)}/></td>
            <td><div><Link className="textLink" href={`/admin/orders/${o.id}`}><strong>#{o.orderNumber}</strong></Link><div className="muted">{new Date(o.createdAt).toLocaleString()}</div></div></td>
            ...
            <td><div className="inline orderActions">
              <Link className="iconBtn" href={`/admin/orders/${o.id}`} title="Open order"><ExternalLink size={15}/></Link>

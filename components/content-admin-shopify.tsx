...
// components/aliexpress-home.tsx) -- new blocks are restricted to them so creating one always
// does something visible, rather than the open-ended type list this used to offer before the
// homepage was hardcoded (task #82), which nothing ever consumed.
const TYPES = ['announcement', 'trust']
const TYPE_LABELS: Record<string, string> = { announcement: 'Announcement bar', trust: 'Trust strip' }
...
export function ContentAdminShopify({ initial, canManage = true }: { initial: Block[]; canManage?: boolean }) {
  ...
  function openCreate() {
    setError(''); setNotice('')
    setEditing({ id: 'new', type: 'announcement', title: '', subtitle: '', contentJson: '{\n  "text": ""\n}', isActive: true, sortOrder: rows.length })
    setCreateOpen(true)
  }

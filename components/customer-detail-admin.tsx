...
import { ArrowLeft, Mail, MapPin, Phone, Save, ShieldOff, UserCheck } from 'lucide-react'
...
export default function CustomerDetailAdmin({ initial }: { initial: any }) {
  ...
  return <div>
    ...
    <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 18, padding: 22, marginBottom: 20 }}><div style={{ width: 64, height: 64, borderRadius: 16, display: 'grid', placeItems: 'center', background: 'var(--soft)', border: '1px solid var(--line)', fontWeight: 900, fontSize: 20 }}>{initials(customer.name)}</div><div><h2 style={{ margin: 0 }}>{customer.name}</h2><div className="muted">Customer since {new Date(customer.createdAt).toLocaleDateString()}</div><div className="inline" style={{ marginTop: 7 }}><span className="muted"><Mail size={14}/> {customer.email}</span>{customer.phone && <span className="muted"><Phone size={14}/> {customer.phone}</span>}</div></div></div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: 18 }}>
      <main style={{ display: 'grid', gap: 18 }}>
        ...
        <section className="editorCard"><div className="editorCardHead"><div><h3>Reviews</h3><p>Reviews written by this customer.</p></div></div><div className="editorCardBody">{!customer.reviews?.length ? <div className="emptyInline">No reviews yet.</div> : customer.reviews.map((r: any) => <div key={r.id} style={{ padding: '13px 0', borderBottom: '1px solid var(--line)' }}><div className="inline" style={{ justifyContent: 'space-between' }}><strong>{r.product?.name}</strong><span>{'★'.repeat(Math.max(0, Math.min(5, r.rating)))}</span></div>{r.title && <strong style={{ display: 'block', marginTop: 5 }}>{r.title}</strong>}<div className="muted" style={{ marginTop: 4 }}>{r.body || 'No review text'}</div></div>)}</div></section>
      </main>
      <aside style={{ display: 'grid', gap: 18, alignContent: 'start' }}>
        <section className="editorCard"><div className="editorCardHead"><div><h3>Contact</h3><p>Quick actions</p></div></div><div className="editorCardBody"><a className="btn secondary" href={`mailto:${customer.email}`}><Mail size={15}/> Email customer</a>{customer.phone && <a className="btn secondary" href={`tel:${customer.phone}`}><Phone size={15}/> Call customer</a>}</div></section>
        <section className="editorCard"><div className="editorCardHead"><div><h3>Addresses</h3><p>Saved customer addresses.</p></div></div><div className="editorCardBody">{!customer.addresses?.length ? <div className="emptyInline">No saved addresses.</div> : customer.addresses.map((a: any) => <div key={a.id} style={{ padding: '12px 0', borderBottom: '1px solid var(--line)' }}><div className="inline"><MapPin size={15}/><strong>{a.label || 'Address'}</strong>{a.isDefault && <span className="pill">Default</span>}</div><div>{a.firstName} {a.lastName}</div><div>{a.line1}{a.line2 ? `, ${a.line2}` : ''}</div><div>{a.city}{a.region ? `, ${a.region}` : ''} {a.postalCode || ''}</div><div>{a.country}</div></div>)}</div></section>

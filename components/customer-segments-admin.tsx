...
export default function CustomerSegmentsAdmin({ initial, canManage }: { initial: CustomerSegment[]; canManage: boolean }) {
  ...
  return <div className={styles.page}>
    <div className={styles.header}>
      <div>
        <Link href="/admin/customers" className="textLink"><ArrowLeft size={13} /> Back to customers</Link>
        <h1 className="h2">Customer segments</h1>
        <p className="muted">Group customers for marketing, discounts and reporting. Assign customers to a segment from their customer profile.</p>
      </div>
      <div className="inline">
        <button className="btn secondary" onClick={refresh} disabled={loading}><RefreshCw size={15} /> Refresh</button>
        {canManage && <button className="btn" onClick={openModal}><Plus size={15} /> New segment</button>}
      </div>
    </div>
        ...
        <table className="table">
          ...
          <tbody>
            {shown.map(seg => <tr key={seg.id}>
              ...
              <td className="muted">{seg.description || '—'}</td>
              <td><span className="pill">{seg._count?.members ?? 0} customers</span></td>
              <td className="muted">{new Date(seg.updatedAt).toLocaleDateString()}</td>
            </tr>)}
          </tbody>
        </table>
      </div>
      {!shown.length && <div className="empty"><UsersRound size={28} /><h3>No segments yet</h3><p className="muted">{canManage ? 'Create a segment to start grouping customers.' : 'Segments created by staff will show up here.'}</p></div>}
    </div>

    {modalOpen && <div className="modalOverlay" onClick={closeModal}>
      <div className={`card ${styles.modal}`} onClick={e => e.stopPropagation()}>
        <div className={styles.modalHead}>
          <div><span className="muted tiny">NEW SEGMENT</span><h2>Create a customer segment</h2><p className="muted">Segments can be assigned to customers individually from their profile.</p></div>
          <button className="iconBtn" onClick={closeModal}><X size={17} /></button>
        </div>

        <div className={styles.modalBody}>
          {modalError && <div className="alert danger">{modalError}</div>}
          <label className="fieldLabel">Name<input className="input" value={name} onChange={e => setName(e.target.value)} placeholder="VIP customers" autoFocus /></label>
          <label className="fieldLabel">Description<textarea className="textarea" rows={2} value={description} onChange={e => setDescription(e.target.value)} placeholder="What this segment represents" /></label>
          <label className="fieldLabel">Rule definition (JSON, optional)<textarea className="textarea" rows={4} value={ruleJson} onChange={e => setRuleJson(e.target.value)} placeholder='{"minOrders": 5}' /></label>
          <div className={styles.modalFooter}>
            <button className="btn secondary" onClick={closeModal} disabled={submitting}>Cancel</button>
            <button className="btn" onClick={createSegment} disabled={submitting}>{submitting ? 'Creating…' : 'Create segment'}</button>
          </div>
        </div>
      </div>
    </div>}

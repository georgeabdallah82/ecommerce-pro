...
export default function ManualOrderForm({ products, customers }:{ products:Product[]; customers:Customer[] }) {
  ...
  return <div>
      ...
      <div className={s.grid}>
        ...
        <aside className={s.rail}><section className="card adminPanel"><div className="sectionHead small"><h3>Order summary</h3></div><div className="summaryLine"><span>Subtotal</span><strong>{money(subtotal)}</strong></div><div className="summaryLine"><span>Discount</span><strong>- {money(discountValue)}</strong></div><div className="summaryLine"><span>Shipping</span><strong>{money(shippingValue)}</strong></div><div className="summaryLine"><span>Tax</span><strong>{money(taxValue)}</strong></div><div className="summaryLine total"><span>Total</span><strong>{money(total)}</strong></div><button className="btn wide" disabled={saving||!lines.length} onClick={createOrder}>{saving?'Creating…':'Create order'}</button></section></aside>
      </div>
  </div>
}

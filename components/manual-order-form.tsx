      ...
      <div className={s.grid}>
        <main className={s.main}>
          <section className="card adminPanel"><div className="sectionHead small"><h3>Customer</h3></div>
            ...
            <div className={s.twoCol}><label className="fieldLabel">Name<input className="input" value={name} onChange={e=>setName(e.target.value)} placeholder="Customer name"/></label><label className="fieldLabel">Email<input className="input" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="customer@email.com"/></label></div>
          ...
          <section className="card adminPanel"><div className="sectionHead small"><h3>Shipping address</h3></div>
            <div className={s.twoCol}><label className="fieldLabel">Country<input className="input" value={country} onChange={e=>setCountry(e.target.value)}/></label><label className="fieldLabel">City<input className="input" value={city} onChange={e=>setCity(e.target.value)}/></label></div>
            ...
            <div className={s.twoCol}><label className="fieldLabel">Region<input className="input" value={region} onChange={e=>setRegion(e.target.value)}/></label><label className="fieldLabel">Postal code<input className="input" value={postalCode} onChange={e=>setPostalCode(e.target.value)}/></label></div>
          </section>

          <section className="card adminPanel"><div className="sectionHead small"><h3>Payment & fulfillment</h3></div>
            <div className={s.twoCol}><label className="fieldLabel">Payment method<select className="input" value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)}><option>COD</option><option>BANK_TRANSFER</option><option>WALLET</option><option>CARD</option></select></label><label className="fieldLabel">Payment status<select className="input" value={paymentStatus} onChange={e=>setPaymentStatus(e.target.value)}><option>UNPAID</option><option>PENDING</option><option>PAID</option><option>FAILED</option></select></label></div>
            <div className={s.twoCol}><label className="fieldLabel">Shipping method<input className="input" value={shippingMethod} onChange={e=>setShippingMethod(e.target.value)}/></label><label className="fieldLabel">Shipping amount<input className="input" type="number" min="0" step="0.01" value={shipping} onChange={e=>setShipping(e.target.value)}/></label></div>
            <div className={s.twoCol}><label className="fieldLabel">Discount<input className="input" type="number" min="0" step="0.01" value={discount} onChange={e=>setDiscount(e.target.value)}/></label><label className="fieldLabel">Tax<input className="input" type="number" min="0" step="0.01" value={tax} onChange={e=>setTax(e.target.value)}/></label></div>
        ...
        <aside className={s.rail}><section className="card adminPanel"><div className="sectionHead small"><h3>Order summary</h3></div><div className={s.summaryLine}><span>Subtotal</span><strong>{money(subtotal)}</strong></div><div className={s.summaryLine}><span>Discount</span><strong>- {money(discountValue)}</strong></div><div className={s.summaryLine}><span>Shipping</span><strong>{money(shippingValue)}</strong></div><div className={s.summaryLine}><span>Tax</span><strong>{money(taxValue)}</strong></div><div className={`${s.summaryLine} ${s.summaryLineTotal}`}><span>Total</span><strong>{money(total)}</strong></div><button className="btn wide" disabled={saving||!lines.length} onClick={createOrder}>{saving?'Creating…':'Create order'}</button></section></aside>
      </div>
  </div>
}

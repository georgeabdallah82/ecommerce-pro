...
import { ChevronDown, ChevronRight, GripVertical, Link2, Minus, Plus, Save, Settings2, Trash2, X } from 'lucide-react'
...
export default function NavigationEditorPro({initial,categories,collections}:Props){
 ...
 const addItem=(type:Item['type']='custom',parentId:string|null=null)=>{const item:Item={id:newId(),label:type==='collection'?'New collection':type==='category'?'New category':'New link',type,url:type==='collection'?'/collections':type==='category'?'/shop':'#',parentId};setItems(cur=>[...cur,item]);setEditing(item)}
 const reorder=(sourceId:string,targetId:string,mode:DropMode)=>{
   ...
   setItems(cur=>{
     ...
     const ordered=[...siblingIds.slice(0,index),sourceId,...siblingIds.slice(index)]
     const byId=new Map(without.map(x=>[x.id,x]))
     const result:Item[]=[];const used=new Set<string>()
     for(const id of ordered){if(id===sourceId){result.push(moved);used.add(id)}else{const item=byId.get(id);if(item){result.push(item);used.add(id)}}}
     for(const item of without){if(!used.has(item.id))result.push(item)}
     return result
   })
   setDragId(null);setOverId(null)
 }
 const moveLevel=(id:string,dir:'indent'|'outdent')=>{const flat=flatten(items);const i=flat.findIndex(x=>x.item.id===id);if(i<0)return;const current=flat[i];if(dir==='indent'){const prev=flat[i-1]?.item;if(!prev)return;patch(id,{parentId:prev.id});return}if(current.item.parentId){const parent=items.find(x=>x.id===current.item.parentId);patch(id,{parentId:parent?.parentId??null})}}
 ...
 return <div className="navProPage">
  <div className="navProHeader"><div><div className="navEyebrow">ONLINE STORE</div><h1 className="h2">Navigation</h1><p className="muted">Build your storefront menu with simple drag-and-drop nesting, just like a modern commerce editor.</p></div><div className="inline"><button className="btn secondary" onClick={()=>setPreviewOpen(v=>!v)}>{previewOpen?'Hide preview':'Show preview'}</button><button className="btn" onClick={save} disabled={saving}><Save size={15}/>{saving?'Saving…':'Save'}</button></div></div>
  ...
  <div className={`navShopifyLayout ${previewOpen?'withPreview':''}`}>
   <section className="navShopifyBuilder card">
    <div className="navMenuHeader"><div><div className="navMenuTitle"><strong>Main menu</strong><span className="navSavedDot">{saving?'Saving…':'Unsaved changes'}</span></div><p className="muted">Your primary storefront navigation</p></div><button className="iconBtn" title="Menu settings"><Settings2 size={16}/></button></div>
    <div className="navToolbar"><button className="btn secondary" onClick={()=>addItem('custom')}><Plus size={15}/> Add menu item</button><div className="navToolbarHint">{roots.length} top-level items · drag the handle to move</div></div>
    <div className="navItemsList">
     ...
     {rows.map(({item,depth})=>{const hasChildren=childrenOf(items,item.id).length>0;const active=overId===item.id;return <div key={item.id} className={`navItemWrap depth-${Math.min(depth,3)}`}>
       <div className={`navShopifyItem ${active?'dropTarget':''}`} style={{marginLeft:depth*24}} onDragOver={e=>{e.preventDefault();if(dragId&&dragId!==item.id&&!descendants(items,dragId).has(item.id))setOverId(item.id)}} onDragLeave={e=>{if(e.currentTarget===e.target)setOverId(null)}} onDrop={e=>{e.preventDefault();const source=e.dataTransfer.getData('text/plain')||dragId;if(source&&source!==item.id&&!descendants(items,source).has(item.id))reorder(source,item.id,'child')}}>
        <span className="navGrip" draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',item.id);setDragId(item.id);setOverId(item.id)}} onDragEnd={()=>{setDragId(null);setOverId(null)}} title="Drag to reorder"><GripVertical size={18}/></span>
        <div className="navItemMain" onClick={()=>setEditing(item)}><strong>{item.label}</strong><span>{item.url||'No link assigned'}</span></div>
        {hasChildren&&<span className="navChildBadge"><ChevronDown size={13}/>Dropdown</span>}
        <button className="navItemAction" title="Add submenu item" onClick={()=>addItem('custom',item.id)}><Plus size={15}/></button>
        <button className="navItemAction" title="Move out one level" onClick={()=>moveLevel(item.id,'outdent')} disabled={!item.parentId}><Minus size={15}/></button>
        <button className="navItemAction" title="Edit" onClick={()=>setEditing(item)}><Settings2 size={15}/></button>
        <button className="navItemAction danger" title="Remove" onClick={()=>remove(item.id)}><Trash2 size={15}/></button>
       </div>
       {active&&dragId&&<div className="navDropActions"><button onClick={()=>reorder(dragId,item.id,'before')}>Place above</button><button onClick={()=>reorder(dragId,item.id,'child')}>Add to {item.label}</button><button onClick={()=>reorder(dragId,item.id,'after')}>Place below</button></div>}
    ...
    <div className="navHelp"><strong>Quick nesting</strong><span>Use + on any item to add a submenu directly, or drag the handle onto another item and choose “Add to”. Click the item name to edit its link.</span></div>
   </section>
   {previewOpen&&<aside className="navPreviewPane card"><div className="navPreviewHeader"><div><strong>Storefront preview</strong><span className="muted">Header navigation</span></div><span className="previewBadge">Desktop</span></div><div className="navPreviewBrowser"><div className="browserDots"><i/><i/><i/></div><div className="previewHeaderBar"><strong>YOUR BRAND</strong><div className="previewLinks">{roots.map(item=><span key={item.id}>{item.label}{childrenOf(items,item.id).length>0&&<ChevronDown size={12}/>}</span>)}</div><div className="previewIcons">♡　⌕　Bag</div></div><div className="previewHero"><span>Storefront preview</span><strong>Build your navigation</strong><small>Dropdowns and nested links update as you edit.</small></div></div></aside>}
  </div>
  {editing&&<div className="navModalOverlay" onMouseDown={()=>setEditing(null)}><div className="navEditPanel" onMouseDown={e=>e.stopPropagation()}><div className="navEditHeader"><div><div className="navEyebrow">MENU ITEM</div><h2>{editing.label||'Menu item'}</h2></div><button className="iconBtn" onClick={()=>setEditing(null)}><X size={17}/></button></div>
    ...
    <div className="navEditTip"><strong>Tip:</strong> Use the + button on a parent item to create submenu entries instantly.</div><div className="navEditFooter"><button className="btn secondary" onClick={()=>setEditing(null)}>Done</button><button className="btn dangerBtn" onClick={()=>{remove(editing.id);setEditing(null)}}>Remove item</button></div>
   </div></div>}
 </div>
}

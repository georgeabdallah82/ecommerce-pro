  ...
  return <div>
    ...
    <div className={s.grid}>
      ...
      <aside className={s.rail}>
        ...
        <section className="card adminPanel"><div className="sectionHead small"><h3>Payment</h3></div><div className={s.summaryLine}><span>Method</span><strong>{o.paymentMethod}</strong></div><div className={s.summaryLine}><span>Status</span><span className={`${s.statusPill} ${paymentPillClass(o.paymentStatus)}`}>{o.paymentStatus}</span></div>{(o.paymentTransactions||[]).map((t:any)=><div key={t.id} className={s.summaryLine}><span>{t.provider}{t.externalId?` • ${t.externalId}`:''}</span><span>{t.status} · {money(t.amount,t.currency)}</span></div>)}</section>

...
export default function ProductListAdminShopify({ initial }: { initial: any }) {
  ...
  useEffect(() => {
    api('/api/admin/categories')
      .then(data => setCategories(Array.isArray(data) ? data : data.rows || data.categories || []))
      .catch(() => undefined)
  }, [])
  ...
  function toggleAll() {
    setSelected(allSelected ? [] : rows.map(product => product.id))
  }
  ...
  async function bulk(action: string) {
    ...
    try {
      const data = await api('/api/admin/products', {
        method: 'POST',
        body: JSON.stringify({ action: 'bulk', ids: selected, bulkAction: action }),
      })
  ...
  return (
    <div className="shopifyCatalogPage">
      <div className="shopifyCatalogHeader">
        <div>
          <div className="shopifyEyebrow">CATALOG</div>
          <div className="shopifyTitleRow">
            <h1 className="shopifyTitle">Products</h1>
            <span className="shopifyCountPill">{total.toLocaleString()}</span>
          </div>
        </div>
      ...
      <div className="shopifyViewBar card">
        <div className="shopifyViews" role="tablist" aria-label="Product views">
          {views.map(([value, label, count]) => (
            <button
              ...
              className={status === value ? 'shopifyView active' : 'shopifyView'}
        ...
        <button type="button" className="btn secondary compactBtn" onClick={() => setFiltersOpen(value => !value)}>
          Filters <ChevronDown className={filtersOpen ? 'rotate180' : ''} size={15} />
        </button>
      </div>

      <div className="card shopifyFilterBar">
        <div className="shopifySearch">
          <Search size={16} />
          <input
            value={q}
        ...
        {filtersOpen && (
          <div className="shopifyFilterGrid">
            <label><span>Category</span><select className="input" value={categoryId} onChange={event => setCategoryId(event.target.value)}><option value="ALL">All categories</option>{categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
            <label><span>Sort</span><select className="input" value={sort} onChange={event => setSort(event.target.value)}><option value="updated_desc">Recently updated</option><option value="created_desc">Newest</option><option value="name_asc">Name A–Z</option><option value="name_desc">Name Z–A</option><option value="price_asc">Price low–high</option><option value="price_desc">Price high–low</option></select></label>
      ...
      {selected.length > 0 && (
        <div className="shopifyBulkBar">
          <div><strong>{selected.length}</strong><span> selected</span></div>
          <div className="shopifyBulkActions">
            ...
            <div className="shopifyActionMenu">
              <button className="btn secondary" onClick={() => setBulkOpen(value => !value)}>More <ChevronDown size={14} /></button>
              {bulkOpen && <div className="shopifyMenu"><button onClick={() => bulk('FEATURED_ON')}><Star size={14} /> Feature products</button><button onClick={() => setSelected([])}><X size={14} /> Clear selection</button></div>}
            </div>
          </div>
        </div>
      )}

      <div className="card shopifyTableCard">
        <div className="shopifyTableMeta">
          <span>{total.toLocaleString()} products</span>
          <label>Rows <select className="input compact" value={pageSize} onChange={event => setPageSize(Number(event.target.value))}><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select></label>
        </div>
        <div className="tableWrap">
          <table className="table shopifyProductTable">
            <thead><tr><th><input aria-label="Select all products" type="checkbox" checked={allSelected} onChange={toggleAll} /></th><th>Product</th><th>Status</th><th>Inventory</th><th>Category</th><th>Price</th><th>Sales channels</th><th>Updated</th><th /></tr></thead>
            <tbody>
              {rows.map(product => {
                const stock = stockInfo(product)
                return (
                  <tr key={product.id} className={selected.includes(product.id) ? 'selectedRow' : ''}>
                    <td><input aria-label={`Select ${product.name}`} type="checkbox" checked={selected.includes(product.id)} onChange={() => toggle(product.id)} /></td>
                    <td>
                      <Link className="shopifyProductCell" href={`/admin/products/${product.id}`}>
                        <div className="shopifyThumb">{product.images?.[0]?.url ? <img src={product.images[0].url} alt="" /> : <span aria-hidden="true">◎</span>}</div>
                        <div><strong>{product.name}</strong><span>{product.sku || 'No SKU'}{product.featured ? ' · Featured' : ''}</span></div>
                      </Link>
                    </td>
                    <td><span className={`shopifyStatus ${String(product.status).toLowerCase()}`}>{product.status}</span></td>
                    <td>{stock.tracked ? <div className={stock.available <= 5 ? 'shopifyStock low' : 'shopifyStock'}><strong>{stock.available}</strong><span>{stock.reserved ? `${stock.reserved} reserved` : 'available'}</span></div> : <span className="muted">Not tracked</span>}</td>
                    ...
                    <td><span className="shopifyChannel">Online Store</span></td>

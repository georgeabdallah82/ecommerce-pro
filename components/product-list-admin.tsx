...
export default function ProductListAdmin({ initial }: { initial: any }) {
  ...
  return <div className="catalogPage">
    <div className="sectionHead catalogHead">
      ...
      <div className="inline"><Link className="btn" href="/admin/products/new"><Plus size={16}/> Add product</Link></div>
    </div>
    ...
    <div className="catalogStats">
      <button className={status === 'ALL' ? 'statCard active' : 'statCard'} onClick={() => setStatus('ALL')}><span className="muted">All products</span><strong>{total}</strong></button>
      <button className={status === 'ACTIVE' ? 'statCard active' : 'statCard'} onClick={() => setStatus('ACTIVE')}><span className="muted">Active</span><strong>{activeCount}</strong></button>
      <button className={status === 'DRAFT' ? 'statCard active' : 'statCard'} onClick={() => setStatus('DRAFT')}><span className="muted">Drafts</span><strong>{draftCount}</strong></button>
      <button className="statCard" onClick={() => setStatus('ALL')}><span className="muted">Low stock in page</span><strong>{lowCount}</strong></button>
    </div>

    <div className="card catalogToolbar">
      <div className="productSearch"><Search size={16}/><input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') void load(1) }} placeholder="Search products, SKUs, vendors…" /><button className="searchClear" hidden={!q} onClick={() => { setQ(''); void load(1) }}><X size={14}/></button></div>
      <button className="btn secondary mobileFilterBtn" onClick={() => setMobileFilter(v => !v)}>Filters <ChevronDown size={15}/></button>
      <div className={mobileFilter ? 'catalogFilters open' : 'catalogFilters'}>
        <select className="input" value={categoryId} onChange={e => setCategoryId(e.target.value)}><option value="ALL">All categories</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
        ...
        <button className="btn secondary" onClick={() => load(1)} disabled={loading}>{loading ? 'Loading…' : 'Search'}</button>
      </div>
    </div>
    ...
    <div className="card productTableCard">
      ...
      <tbody>{rows.map(p => { const s = stockInfo(p); const featured = Boolean(p.featured); return <tr key={p.id} className={selected.includes(p.id) ? 'selectedRow' : ''}>
        ...
        <td><span className={`statusPill ${String(p.status).toLowerCase()}`}>{p.status === 'ACTIVE' ? <CheckCircle2 size={13}/> : p.status === 'DRAFT' ? <Tag size={13}/> : <Archive size={13}/>} {p.status}</span></td>
        <td><div className={s.available <= 5 && s.tracked ? 'stockCell low' : 'stockCell'}>{s.tracked ? <><strong>{s.available}</strong><span className="muted">{s.reserved ? `${s.reserved} reserved` : 'available'}</span></> : <span className="muted">Not tracked</span>}</div></td>

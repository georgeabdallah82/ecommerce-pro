...
type PaymentConfig = { provider:string; enabled:boolean; merchantId:string; merchantName:string; apiBaseUrl:string; apiVersion:string; checkoutScriptUrl:string; hasApiPassword:boolean; apiPassword:string }
const defaultPaymentConfig:PaymentConfig={provider:'manual',enabled:false,merchantId:'',merchantName:'',apiBaseUrl:'https://epayment.areeba.com/api/rest',apiVersion:'78',checkoutScriptUrl:'https://epayment.areeba.com/static/checkout/checkout.min.js',hasApiPassword:false,apiPassword:''}
...
export default function SettingsCenterPro({ initial }: { initial: any[] }) {
  ...
  useEffect(()=>{ if(tab!=='Payments') return; setPaymentLoading(true); api('/api/admin/payment-provider').then(d=>setPaymentConfig({...defaultPaymentConfig,...d,apiPassword:''})).catch(e=>setNotice(e instanceof Error?e.message:'Unable to load payment provider settings')).finally(()=>setPaymentLoading(false)) },[tab])
  ...
  return <div className="settingsPro">
    <style jsx global>{`.settingsPro{max-width:1180px;margin:0 auto}.settingsPro .settingsProHead{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:22px}.settingsPro .settingsSearch{width:260px;height:42px;display:flex;align-items:center;gap:8px;padding:0 12px;border:1px solid var(--line);border-radius:10px;background:#fff}.settingsPro .settingsSearch input{border:0;outline:0;flex:1;min-width:0}.settingsPro .settingsProLayout{display:grid;grid-template-columns:270px minmax(0,1fr);gap:20px;align-items:start}.settingsPro .settingsProNav{display:grid;gap:4px;position:sticky;top:24px}.settingsPro .settingsProNav button{display:grid;grid-template-columns:22px 1fr 16px;align-items:center;gap:8px;width:100%;padding:11px 12px;border:1px solid transparent;background:transparent;border-radius:10px;color:#555;text-align:left;cursor:pointer}.settingsPro .settingsProNav button:hover{background:#fff;border-color:var(--line)}.settingsPro .settingsProNav button.active{background:#fff;border-color:var(--line);color:var(--ink);box-shadow:0 1px 2px rgba(0,0,0,.03)}.settingsPro .settingsProNav span{display:grid;gap:2px}.settingsPro .settingsProNav strong{font-size:13px}.settingsPro .settingsProNav small{font-size:11px;color:var(--muted);line-height:1.25}.settingsPro .settingsProMain{display:grid;gap:18px}.settingsPro .settingsProCard{background:#fff;border:1px solid var(--line);border-radius:14px;overflow:hidden}.settingsPro .settingsProCard header{padding:20px;border-bottom:1px solid var(--line)}.settingsPro .settingsProCard h2{margin:0;font-size:18px}.settingsPro .settingsProCard header p{margin:5px 0 0;font-size:13px}.settingsPro .settingsProBody{padding:20px;display:grid;gap:18px}.settingsPro .settingsToggle{display:flex;justify-content:space-between;align-items:center;gap:18px;padding:14px 0;border-bottom:1px solid var(--line)}.settingsPro .settingsToggle:last-child{border-bottom:0}.settingsPro .settingsToggle>div{display:grid;gap:3px}.settingsPro .settingsToggle strong{font-size:14px}.settingsPro .settingsToggle small{color:var(--muted);font-size:11px}.settingsPro .settingsToggle button{width:44px;height:24px;border:0;border-radius:999px;background:#c8c8c8;padding:3px;cursor:pointer;transition:.18s}.settingsPro .settingsToggle button i{display:block;width:18px;height:18px;border-radius:50%;background:#fff;transition:.18s}.settingsPro .settingsToggle button.on{background:#1c7d45}.settingsPro .settingsToggle button.on i{transform:translateX(20px)}.settingsPro .marketSummary{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.settingsPro .marketSummary>div{padding:16px;border:1px solid var(--line);border-radius:12px;display:grid;gap:4px}.settingsPro .marketSummary span{font-size:12px;color:var(--muted)}.settingsPro .securityCallout{display:flex;gap:12px;padding:16px;border:1px solid #dfe8e2;background:#f5faf6;border-radius:12px}.settingsPro .securityCallout p{margin:4px 0 0}@media(max-width:800px){.settingsPro .settingsProHead{align-items:stretch;flex-direction:column}.settingsPro .settingsProLayout{grid-template-columns:1fr}.settingsPro .settingsProNav{position:static;display:flex;overflow:auto;padding-bottom:4px}.settingsPro .settingsProNav button{min-width:220px}.settingsPro .settingsSearch{width:100%}.settingsPro .marketSummary{grid-template-columns:1fr}.settingsPro .settingsProHead .inline{align-items:stretch}.settingsPro .settingsProHead .btn{width:100%}}`}</style>
    <div className="settingsProHead"><div><span className="muted tiny">SETTINGS</span><h1 className="h2">Settings</h1><p className="muted">Manage your store from one place.</p></div><div className="inline"><div className="settingsSearch"><Search size={15}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search settings"/></div><button className="btn" onClick={save} disabled={saving||paymentLoading}><Save size={15}/>{saving ? 'Saving…' : 'Save'}</button></div></div>
    ...
    <div className="settingsProLayout">
      ...
      <main className="settingsProMain">
        ...
        {tab==='Payments' && <>
          ...
          <Card title="Online payment provider" desc="Connect and switch gateways without changing checkout code."><label className="fieldLabel">Provider<select className="input" value={paymentConfig.provider} onChange={e=>setPaymentConfig(v=>({...v,provider:e.target.value}))}><option value="manual">Manual / no online gateway</option><option value="areeba_mpgs">Areeba Hosted Checkout (MPGS)</option></select></label><div className="twoColFields"><Field label="Merchant ID" value={paymentConfig.merchantId} onChange={v=>setPaymentConfig(c=>({...c,merchantId:v}))}/><Field label="Merchant name" value={paymentConfig.merchantName} onChange={v=>setPaymentConfig(c=>({...c,merchantName:v}))}/><Field label="Gateway API base URL" value={paymentConfig.apiBaseUrl} onChange={v=>setPaymentConfig(c=>({...c,apiBaseUrl:v}))}/><Field label="API version" value={paymentConfig.apiVersion} onChange={v=>setPaymentConfig(c=>({...c,apiVersion:v}))}/><Field label="Checkout script URL" value={paymentConfig.checkoutScriptUrl} onChange={v=>setPaymentConfig(c=>({...c,checkoutScriptUrl:v}))}/><label className="fieldLabel">API password<input className="input" type="password" value={paymentConfig.apiPassword} onChange={e=>setPaymentConfig(c=>({...c,apiPassword:e.target.value}))} placeholder={paymentConfig.hasApiPassword?'Saved — leave blank to keep it':'Enter gateway API password'} autoComplete="new-password" /></label></div><div className="securityCallout"><ShieldCheck size={20}/><div><strong>Gateway secret protected</strong><p className="muted">The API password is encrypted before it is stored and is never returned to the admin UI.</p></div></div></Card>
        </>}
...
function Toggle({label,keyName,value,onChange}:{label:string;keyName:string;value:boolean;onChange:()=>void}){return <div className="settingsToggle"><div><strong>{label}</strong><small>{keyName}</small></div><button type="button" aria-label={label} className={value?'on':''} onClick={onChange}><i/></button></div>}

...
export default function ShopifyThemeEditorV3({ initial }: { initial: { theme: Theme; sections: Section[]; navigation: any[] } }) {
  ...
  useEffect(() => {
    let active = true
    Promise.all([
      fetch('/api/products?page=1&pageSize=100', { cache: 'no-store' }).then((r) => r.ok ? r.json() : { rows: [] }).catch(() => ({ rows: [] })),
      fetch('/api/admin/collections', { cache: 'no-store' }).then((r) => r.ok ? r.json() : []).catch(() => []),
    ]).then(([productData, collectionData]) => {
      if (!active) return
      setProducts(Array.isArray(productData) ? productData : Array.isArray(productData?.rows) ? productData.rows : [])
      setCollections(Array.isArray(collectionData) ? collectionData : Array.isArray(collectionData?.rows) ? collectionData.rows : [])
    })
    return () => { active = false }
  }, [])
  ...
  const commit = (next: Snapshot, record = true) => {
    if (record) {
  ...
  const mutate = (fn: (current: Snapshot) => Snapshot) => commit(fn(snapshot()))
  ...
  const removeSection = () => {
    ...
    setSelectedId(remaining[Math.min(Math.max(0, selectedIndex - 1), Math.max(remaining.length - 1, 0))]?.id || remaining[0]?.id || '')
    setDrawerOpen(false)
  }
  ...
  const undo = () => {
    ...
    setFuture((items) => [...items, current].slice(-50))
    setHistory((items) => items.slice(0, -1))
    setTheme(previous.theme); setTemplate(previous.template); setSections(previous.sections); setNavigation(previous.navigation); setSelectedId(previous.sections[0]?.id || ''); setDirty(true)
  }
  const redo = () => {
    ...
    setTheme(next.theme); setTemplate(next.template); setSections(next.sections); setNavigation(next.navigation); setSelectedId(next.sections[0]?.id || ''); setDirty(true)
  }
  ...
  return <div className={styles.editor}>
    <header className={styles.topbar}>
      <div className={styles.topLeft}><a className={styles.iconButton} href="/admin/online-store"><ArrowLeft size={17}/></a><div><div className={styles.eyebrow}>ONLINE STORE</div><h1>Theme editor</h1><select className={styles.templateSelect} value={template} onChange={(event) => switchTemplate(event.target.value)}>{TEMPLATES.map((item) => <option key={item}>{item}</option>)}</select></div></div>
      <div className={styles.topActions}>{dirty && <span className={styles.unsaved}><i/>Unsaved changes</span>}<button className={styles.iconButton} aria-label="Undo" disabled={!history.length || saving} onClick={undo}><Undo2 size={16}/></button><button className={styles.iconButton} aria-label="Redo" disabled={!future.length || saving} onClick={redo}><Redo2 size={16}/></button><div className={styles.deviceSwitch}>{(['desktop','tablet','mobile'] as const).map((item) => <button key={item} className={device === item ? styles.active : ''} aria-label={`${item} preview`} onClick={() => setDevice(item)}>{item === 'desktop' ? <Monitor size={15}/> : item === 'tablet' ? <Tablet size={15}/> : <Smartphone size={15}/>}</button>)}</div><button className={styles.secondaryButton} onClick={() => window.open('/', '_blank', 'noopener,noreferrer')}><Eye size={15}/>View live</button><button className={styles.primaryButton} disabled={saving || !dirty} onClick={save}><Save size={15}/>{saving ? 'Saving…' : 'Save'}</button></div>
    </header>
    <div className={styles.layout}>
      <aside className={styles.sidebar}>
        <div className={styles.sidebarHead}><div><div className={styles.eyebrow}>{template}</div><h2>Sections</h2><p>{sections.filter((section) => section.enabled !== false).length} visible · {sections.length} total</p></div><button className={styles.iconButton} aria-label="Add section" onClick={() => setPickerOpen(true)}><Plus size={17}/></button></div>
        <div className={styles.sectionList}>{sections.map((section, index) => <div key={section.id} draggable onDragStart={() => setDragId(section.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropSection(section.id)} className={`${styles.sectionRow} ${selectedId === section.id ? styles.selected : ''} ${section.enabled === false ? styles.disabled : ''}`}><button type="button" className={styles.dragHandle} title="Drag to reorder"><GripVertical size={15}/></button><button type="button" className={styles.sectionMain} onClick={() => { setSelectedId(section.id); setDrawerOpen(true) }}><span>{labelFor(section.type)}</span>{index === 0 && <em>MAIN</em>}</button><button type="button" className={styles.visibilityButton} aria-label={section.enabled === false ? `Enable ${labelFor(section.type)}` : `Disable ${labelFor(section.type)}`} onClick={(event) => { event.stopPropagation(); toggleSection(section.id) }}>{section.enabled === false ? '○' : '●'}</button></div>)}</div>
        <button className={styles.addSection} type="button" onClick={() => setPickerOpen(true)}><Plus size={15}/>Add section</button>
      ...
      <main className={`${styles.preview} ${styles[device]}`}><div className={styles.previewBar}><div><span>{dirty ? 'DRAFT PREVIEW' : 'LIVE PREVIEW'}</span><strong>{template}</strong></div><span className={styles.live}><i/>{dirty ? 'Unsaved draft' : 'Saved storefront'}</span></div><div className={styles.canvas}><div className={styles.store}><div className={styles.announcement}>{announcement}</div><StorefrontSections sections={visibleSections} theme={theme} products={products} collections={collections}/></div></div></main>
      {drawerOpen && selected && <aside className={styles.drawer} role="dialog" aria-modal="true"><div className={styles.drawerHead}><div><span>SECTION</span><h2>{labelFor(selected.type)}</h2></div><div className={styles.drawerActions}><button aria-label="Move section up" onClick={() => moveSelected(-1)} disabled={selectedIndex <= 0}><span>↑</span></button><button aria-label="Move section down" onClick={() => moveSelected(1)} disabled={selectedIndex >= sections.length - 1}><span>↓</span></button><button aria-label="Duplicate section" onClick={duplicateSection}><Copy size={14}/></button><button aria-label="Delete section" onClick={removeSection} className={styles.danger}><Trash2 size={14}/></button><button aria-label="Close" onClick={() => setDrawerOpen(false)}><X size={15}/></button></div></div><div className={styles.drawerBody}><Field label="Heading" value={selected.settings?.heading || ''} onChange={(value) => updateSelected({ heading: value })}/><TextArea label="Text" value={selected.settings?.text || ''} onChange={(value) => updateSelected({ text: value })}/><Field label="Button label" value={selected.settings?.buttonLabel || ''} onChange={(value) => updateSelected({ buttonLabel: value })}/><Field label="Button URL" value={selected.settings?.buttonUrl || ''} onChange={(value) => updateSelected({ buttonUrl: value })}/><Select label="Columns" value={String(selected.settings?.columns || 4)} options={['2','3','4','5','6']} onChange={(value) => updateSelected({ columns: Number(value) })}/><Range label="Spacing" value={Number(selected.settings?.spacing || 72)} min={0} max={160} onChange={(value) => updateSelected({ spacing: value })}/></div></aside>}
    </div>
    {pickerOpen && <div className={styles.modalBackdrop} role="dialog" aria-modal="true" onMouseDown={() => setPickerOpen(false)}><div className={styles.picker} onMouseDown={(event) => event.stopPropagation()}><div className={styles.pickerHead}><div><span>ADD SECTION</span><h2>Build {template}</h2></div><button className={styles.iconButton} aria-label="Close section picker" onClick={() => setPickerOpen(false)}><X size={16}/></button></div><div className={styles.search}><Search size={15}/><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search sections"/></div>{pickerGroups.map((group) => <div className={styles.pickerGroup} key={group.category}><span>{group.category}</span>{group.items.map((key) => <button key={key} onClick={() => addSection(key)}><div><strong>{META[key].label}</strong><small>{META[key].description}</small></div><Plus size={15}/></button>)}</div>)}{!pickerGroups.length && <div className={styles.emptyPicker}>No sections match “{query}”.</div>}</div></div>}

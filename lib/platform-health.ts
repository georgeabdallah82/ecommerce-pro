...
export async function runPlatformHealth(): Promise<{ status: HealthSeverity; checks: HealthCheck[]; generatedAt: string }> {
  ...
  const [staff, products, activeProducts, variants, inventory, orders, pendingOrders, customers, coupons, shippingZones, shippingRates, themeSettings, navigationSettings, pages, blogPosts, media, audits, locations, activeLocations, transfers, draftOrders, openDraftOrders, fulfillments, purchaseOrders, openPurchaseOrders, giftCards, webhooks, activeWebhooks, apiCredentials, activeApiCredentials, salesChannels, activeSalesChannels, abandonedCheckouts, openAbandonedCheckouts, orderEdits, openOrderEdits, expiredActiveCoupons, activeWithoutPublishDate, zonesWithoutRates] = await Promise.all([
    db.user.count({ where: { role: { not: 'CUSTOMER' }, isActive: true } }), db.product.count(), db.product.count({ where: { status: 'ACTIVE' } }), db.productVariant.count(), db.inventoryItem.findMany({ select: { quantity: true, reserved: true, lowStockThreshold: true } }), db.order.count(), db.order.count({ where: { status: 'PENDING' } }), db.user.count({ where: { role: 'CUSTOMER' } }), db.coupon.count({ where: { isActive: true } }), db.shippingZone.count({ where: { isActive: true } }), db.shippingRate.count({ where: { isActive: true } }), db.setting.count({ where: { key: { in: ['theme.config', 'theme.sections'] } } }), db.setting.count({ where: { key: 'navigation.main' } }), db.page.count(), db.blogPost.count(), db.mediaAsset.count(), db.auditLog.count(), db.storeLocation.count(), db.storeLocation.count({ where: { status: 'ACTIVE' } }), db.inventoryTransfer.count(), db.draftOrder.count(), db.draftOrder.count({ where: { status: { in: ['DRAFT', 'OPEN'] } } }), db.fulfillment.count(), db.purchaseOrder.count(), db.purchaseOrder.count({ where: { status: { in: ['DRAFT', 'ORDERED', 'PARTIALLY_RECEIVED'] } } }), db.giftCard.count({ where: { status: 'ACTIVE' } }), db.webhookEndpoint.count(), db.webhookEndpoint.count({ where: { status: 'ACTIVE' } }), db.apiCredential.count(), db.apiCredential.count({ where: { status: 'ACTIVE' } }), db.salesChannel.count(), db.salesChannel.count({ where: { status: 'ACTIVE' } }), db.abandonedCheckout.count(), db.abandonedCheckout.count({ where: { status: 'OPEN' } }), db.orderEdit.count(), db.orderEdit.count({ where: { status: 'OPEN' } }),
    db.coupon.count({ where: { isActive: true, expiresAt: { lt: new Date() } } }), db.product.count({ where: { status: 'ACTIVE', publishedAt: null } }), db.shippingZone.count({ where: { isActive: true, rates: { none: { isActive: true } } } }),
  ])
  ...
  checks.push({ key: 'theme', label: 'Theme system', severity: 'ok', message: themeSettings > 0 ? 'Theme has been customized from defaults.' : 'Using default theme configuration.' })
  checks.push({ key: 'navigation', label: 'Navigation', severity: 'ok', message: navigationSettings > 0 ? 'Navigation has been customized from defaults.' : 'Using default navigation menu.' })

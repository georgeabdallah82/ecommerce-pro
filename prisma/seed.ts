...
async function main(){
 ...
 for(const [name,slug,price,sku,feat] of products){const p=await db.product.upsert({where:{slug},update:{basePrice:price,status:ProductStatus.ACTIVE,featured:feat},create:{name,slug,basePrice:price,sku,status:ProductStatus.ACTIVE,featured:feat,shortDescription:'Thoughtfully selected essentials with a clean, premium presentation.',categoryId:sku==='SKU-004'?home.id:featured.id,images:{create:{url:'/placeholder-product.svg',alt:name}},inventory:{create:{quantity:24,lowStockThreshold:5}}}});await db.product.update({where:{id:p.id},data:{categoryId:sku==='SKU-004'?home.id:featured.id}})}
